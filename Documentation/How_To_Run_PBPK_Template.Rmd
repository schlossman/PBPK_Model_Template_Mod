...
1.	Install R, RStudio, and the R packages "MCSimMod" and "readxl", and Rtools, on your system. Instructions can be found in "Software_installation_instructions.html" in this folder.

2.	Open RStudio.

3.  Optional: open the file, "run_template_model.R" and the "Source" button to run the included commands and load the included functions. This will load the MCSimMod and readx libraries, create the "template" MCSimMod model object, and define the key R functions needed to run the model. The various chemical-specific .R files should also "source" "run_template_model.R" as an initial step, but it may be helpful to do this yourself first, e.g., if trying to perform other analyses.

4.	Within RStudio, open the R source code file containing the desired simulations, e.g. "DCM_IRIS_scripts.R" and use the "Source" button to load the included functions.
...
1.	Install R, Rtools, RStudio (optional but suggested), and the R packages "MCSimMod" and "readxl" (with dependent packages) on your system. Detailed instructions can be found in "Software_Installation_Instructions".

1.	Open RStudio or R.

1.  Open "run_template_model.R" and "Source" the file to load the included functions. This should also set your working directory to the directory containing this file which is required for the functions to work.
...
1.  *Store these input spreadsheets in the "Inputs" folder!*

1.  Run the desired simulation by calling the function PBPK_run:
` [Your output dataframe] <- PBPK_run(`
