...
plot.Kim <- function(out, out.inc=NULL, chem, sex, dose, img.name=NULL, Pylims=NULL, 
  ...
  if (!is.null(out.inc)) {
    print.noquote(paste("Maximum absolute relative difference, blood:",
                        max(abs(out.inc[,"C_ven"] - out[,"C_bl"])/max(out[,"C_bl"]))))
    print.noquote(paste("Maximum absolute relative difference, liver:",
                        max(abs(out.inc[,"C_li"] - out[,"C_li"])/max(out[,"C_li"]))))
    print.noquote(paste("Maximum absolute relative difference, kidney:",
                        max(abs(out.inc[,"C_ki"] - out[,"C_ki"])/max(out[,"C_ki"]))))
    print.noquote(paste("Maximum absolute relative difference, urine:",
                        max(abs(out.inc[,"A_urine"] - out[,"A_urine"])/max(out[,"A_urine"]))))
