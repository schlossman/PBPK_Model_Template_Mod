...
plot.Kim <- function(out, out.inc=NULL, chem, sex, dose, img.name=NULL, Pylims=NULL, 
  ...
  if (!is.null(out.inc)) {
    if (test_univ){
      print.noquote(paste("Maximum absolute percent difference (3 sig figs), blood:",
                          max.diff(out.inc$C_ven, out$C_bl)))
      print.noquote(paste("Maximum percent relative difference (3 sig figs), liver:",
                          max.diff(out.inc$C_li, out$C_li)))
      print.noquote(paste("Maximum percent relative difference (3 sig figs), kidney:",
                          max.diff(out.inc$C_ki, out$C_ki)))
      print.noquote(paste("Maximum percent relative difference (3 sig figs), urine:",
                          max.diff(out.inc$A_urine, out$A_urine)))
    } else {
      print.noquote(paste("Maximum absolute difference, relative to Cmax (3 sig figs), blood:",
                          max.diff.scale(out.inc$C_bl, out$C_bl, scale=max(out$C_bl))))
      print.noquote(paste("Maximum absolute difference, relative to Cmax (3 sig figs), liver:",
                          max.diff.scale(out.inc$C_li, out$C_li, scale=max(out$C_li))))
      print.noquote(paste("Maximum absolute difference, relative to Cmax (3 sig figs), kidney:",
                          max.diff.scale(out.inc$C_ki, out$C_ki, scale=max(out$C_ki))))
      print.noquote(paste("Maximum absolute difference, relative to Amax (3 sig figs), urine:",
                          max.diff.scale(out.inc$A_urine, out$A_urine, scale=max(out$A_urine)))) 
    }
  }
  ...
  lines(out.inc$time.days, (out.inc$C_bl+out.inc$C_ven)*C.convert, lty=inc.templ.lty, 
        lwd=3, col=inc.templ.col)
  ...
  legend("bottomright", legend=c(second_sim_legend, 
...
plot.Kudo.Loccisano <- function(out, out.inc=NULL, dose=NULL, img.name=NULL,
                                second_sim_legend=NULL){
  ...
  lines(out.inc$time, (out.inc$C_bl+out.inc$C_ven), lty=inc.templ.lty, lwd=3, col=inc.templ.col)
  title("Plasma", line=-1.25)
  ...
  lines(out.inc$time, out.inc$C_li, lty=inc.templ.lty, lwd=3, col=inc.templ.col)
  title("Liver", line=-1.25)
...
#         lty=c(NA, paper.lty, cor.templ.lty), pch=c(19,NA,NA))
  legend("bottomleft", legend=c("Published Data", "Published Model"),
         xpd=TRUE, inset=c(0.12, 0), bty="n", col=c(paper.col, paper.col),
         lty=c(NA, paper.lty), pch=c(19,NA), lwd=c(1,2))
  legend("bottomright", legend=c("Template Version", second_sim_legend),
         xpd=TRUE, inset=c(0.03, 0), bty="n", col=c(cor.templ.col, inc.templ.col),
         lty=c(cor.templ.lty, inc.templ.lty), pch=c(NA,NA), lwd=c(3,3))
  
  par(mfrow=c(1, 2), mar=c(3, 3, 0, 0), oma=c(2, 0, 1, 1),  mgp=c(1.5, 0.4, 0))

  plot(out$time, out$A_urine, lty=cor.templ.lty, col=cor.templ.col, lwd=3, type="l")
  lines(out.inc$time, out.inc$A_urine, lty=inc.templ.lty, lwd=3, col=inc.templ.col)
  title("Urine", line=-1.25)
  
  plot(out$time, out$A_bal, lty=cor.templ.lty, col=cor.templ.col, lwd=3, type="l",
       ylim=c(0,0.0012))
  lines(out.inc$time, out.inc$A_bal, lty=inc.templ.lty, lwd=3, col=inc.templ.col)
  title("Mass Balance", line=-1.25)
...
plot.Kemper.Loccisano <- function(out, out.inc=NULL, dose.type=NULL, dose=NULL, 
                                  sex=NULL, img.name=NULL, second_sim_legend=NULL){
  # Saves as a tif with name img.name if option is included.
  # Note, you must include the ".tif" in the image name 
  ...
  lines(out$time, out$C_bl, lty=cor.templ.lty, col=cor.templ.col, lwd=3)
  lines(out.inc$time, (out.inc$C_bl+out.inc$C_ven), lty=inc.templ.lty, lwd=3, 
        col=inc.templ.col)
  title("Plasma", line=-1.25)
  
  if (dose.type == "oral"){
    urine.percent <- (out$A_urine/max(out$A_in))*100
    feces.percent <- (out$A_fecal/max(out$A_in))*100
    ...
    if (!is.null(out.inc)){
      lines(out.inc$time, (out.inc$A_urine/max(out.inc$A_in))*100, 
            lty=inc.templ.lty, lwd=3, col=inc.templ.col)    
      lines(out.inc$time, (out.inc$A_fecal/max(out.inc$A_in))*100, 
            lty=inc.templ.lty, lwd=3, col=inc.templ.col)
    }
    title("Urine", line=-1.25)
...
plot.3M.Loccisano <- function(out, out.inc=NULL, sex="male", img.name=NULL,
                              second_sim_legend = NULL){
  # Saves as a tif with name img.name if option is included.
  # Note, you must include the ".tif" in the image name 
  ...
  lines(out.inc$time.days, (out.inc$C_bl+out.inc$C_ven), lty=inc.templ.lty,
        lwd=3, col=inc.templ.col)
  ...
  lines(out.inc$time.days, (out.inc$A_li + out.inc$A_lib)/out.inc$V_li,
        lty=inc.templ.lty, lwd=3,  col=inc.templ.col)
  title("Liver", line=-1.25)
